		...
		while (true) {
			...
			case UNSIGNED_INTEGER_STATE: {
				...
				if (!current_char.has_value()) {
					...
					ss >> token;
					//除去首部的0
					while (token[0]=='0' && token.size() > 1)token.erase(0, 1);
					//判断是否溢出
					if (token.size() > 10)
						...
						if (token.compare("2147483647") > 0)
							return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos, ErrorCode::ErrIntegerOverflow));
					
					return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER, token, pos, currentPos()), std::optional<CompilationError>());
				}
			...
			case IDENTIFIER_STATE: {
				...
				if (!current_char.has_value()) {
					std::string token;
					ss >> token;
					if (token == "begin")result.first = Token(BEGIN, token, pos, currentPos());
					else if (token == "end")result.first = Token(END, token, pos, currentPos());
					else if (token == "var")result.first = Token(VAR, token, pos, currentPos());
					else if (token == "const")result.first = Token(CONST, token, pos, currentPos());
					else if (token == "print")result.first = Token(PRINT, token, pos, currentPos());
					else result.second = checkToken(miniplc0::Token(IDENTIFIER, token, pos, currentPos()));
					if (!result.second.has_value() && !result.first.has_value())result.first= Token(IDENTIFIER, token, pos, currentPos());
					return result;
				}
				auto ch = current_char.value();
