...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		while (true){
			...
			if (!next.has_value())
				...
				addUninitializedVariable(hold.value());
				unreadToken();
			}
			// '='
			// '<表达式>'
				...
				if (err.has_value())
					return err;
				addVariable(hold.value());
				_instructions.emplace_back(STO, getIndex(hold.value().GetValueString()));
			}
			// ';'
			next = nextToken();
			if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		if(next.value().GetType() != TokenType::UNSIGNED_INTEGER)
			return std::make_optional<CompilationError>(_current_pos, ErrConstantNeedValue);
		out = stoi(next.value().GetValueString());
		out = out * op;
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		if(next.value().GetType() != TokenType::EQUAL_SIGN)
			return std::make_optional<CompilationError>(_current_pos, ErrInvalidInput);
		auto err = analyseExpression();
		if (err.has_value())
			return err;
		if (isUninitializedVariable(hold.value().GetValueString())) {
			_vars.insert(std::pair<std::string, int32_t>(hold.value().GetValueString(), _uninitialized_vars[hold.value().GetValueString()]));
			_uninitialized_vars.erase(hold.value().GetValueString());
		}
	...
	std::optional<CompilationError> Analyser::analyseFactor() {
		...
		case UNSIGNED_INTEGER:
			_instructions.emplace_back(LIT, stoi(next.value().GetValueString()));
			break;
		case LEFT_BRACKET:
